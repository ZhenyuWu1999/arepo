#!/bin/bash
# Forced-two-bin validation of the moving production-form candidate:
# N + contour + co-moving + Arpaia.  This is not the natural multi-bin release.

TWODIMS
GAMMA=1.4
READ_MASS_AS_DENSITY_IN_INPUT

CREATE_FULL_MESH
REGULARIZE_MESH_CM_DRIFT
REGULARIZE_MESH_CM_DRIFT_USE_SOUNDSPEED
REGULARIZE_MESH_FACE_ANGLE

RESIDUAL_DISTRIBUTION
N_SCHEME
RD_RK2_TOTAL_RESIDUAL
RD_HIERARCHICAL_TIMESTEPS
RD_HIERARCHICAL_TEST_PATTERN
RD_HIER_TEST_SPLIT_FRAC=0.5
RD_ALE_HIERARCHICAL
RD_ALE_HIERARCHICAL_ARPAIA
RD_ALE_CONTOUR_RESIDUAL
RD_ELEMENT_COMOVING_FRAME

TREE_BASED_TIMESTEPS

DOUBLEPRECISION=1
INPUT_IN_DOUBLEPRECISION
OUTPUT_IN_DOUBLEPRECISION
OUTPUT_CENTER_OF_MASS
OUTPUT_TIMEBIN_HYDRO
OUTPUT_VERTEX_VELOCITY
OUTPUT_VOLUME
OUTPUT_PRESSURE

HAVE_HDF5
