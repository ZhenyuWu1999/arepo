...
def environment_stamp() -> str:
    """The library versions the byte content actually depends on.
    ...
    reproducible everywhere. Recording the environment makes a --verify
    mismatch diagnosable instead of merely alarming.
    """
    return f"numpy {np.__version__}, h5py {h5py.__version__}, hdf5 {h5py.version.hdf5_version}"
    ...
    for line in MANIFEST.read_text().splitlines():
        ...
        if line.startswith("#"):
            if "generated by" in line:
                ...
                if recorded != current:
                    print(f"  note     manifest was written under {recorded};"
                          f" this is {current}")
            continue
...
def main() -> int:
    ...
    existing = sorted(p.name for p in HERE.glob("IC_*.hdf5"))
